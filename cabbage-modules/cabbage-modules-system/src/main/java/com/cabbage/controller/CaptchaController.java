...
public class CaptchaController {
    ...
    public void getCaptcha(@PathVariable("uuid") String uuid, HttpServletRequest request, HttpServletResponse response) {
        ...
        session.setAttribute(Captcha.RESPONSE_KEY_UUID, uuid);
        captcha.buildCaptcha(session, response);
        Object attribute = session.getAttribute(Captcha.SESSION_KEY);
        String captchaKey = redisKey.forBiz("captchaKey", uuid);
