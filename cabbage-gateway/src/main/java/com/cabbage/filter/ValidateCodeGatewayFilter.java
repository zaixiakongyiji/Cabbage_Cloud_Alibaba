	...
	public GatewayFilter apply(Object config) {
		return (exchange, chain) -> {
			...
			boolean isAuthToken = CharSequenceUtil.containsAnyIgnoreCase(request.getURI().getPath(),
					SecurityConstants.OAUTH_LOGIN_URL);
			...
			boolean isIgnoreClient = configProperties.getIgnore().contains(WebUtils.getClientId(request));
			try {
				...
				return response.writeWith(Mono.create(monoSink -> {
					try {
						byte[] bytes = objectMapper.writeValueAsBytes(Result.ero(errMsg));
